...
\scalebox{0.79}{
  ...
  biogram:::ggplot2.criterion_distribution(tmp) + poster_theme +
    annotation_custom(grob = tableGrob(xyTable, 
                                       gpar.coretext = gpar(cex = 2),
                                       gpar.coltext = gpar(cex = 1.8,  col = " firebrick3"),
                                       gpar.rowtext = gpar(cex = 1.8, col = "firebrick3")),
                      xmin = 10, xmax = 40, ymin = 0.1, ymax = 0.8) 
  @
}
\end{columns}
\end{block}
...
It is especially useful when very precise p-values are required and features are sparse vectors.
\end{block}
\vfill 
