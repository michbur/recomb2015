...
@ 
    ...
    3816 eukaryotic proteins with experimentally confirmed signal peptides and 9795 eukaryotic proteins without signal peptides (UniProtKB 2014\_07).
...
The mean absolute cleavage site misplacement (MAPD) is \Sexpr{round(poster_data[["mean"]], 4)}.
...
colnames(benchmark_dat) <- c("AUC", "H-measure", "Gini index", "MAPD")
xtable(benchmark_dat, align = c("r", rep("c", 4)))
@ 
    \end{block}
    \vfill
     
    \begin{block}{Summary}
      Hidden semi-Markov models can be used to accurately predict the presence of secretory signal peptides effectively extracting information from very data sets.
    \end{block}
    \vfill 
