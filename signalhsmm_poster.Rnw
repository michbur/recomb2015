...
\begin{frame}
  \begin{columns}
    ...
    \begin{block}{Goal of the project}
      Develop an accurate method of predicting signal peptide presence.
    \end{block}
    \vfill
    ...
    \begin{block}{Training  of the algorithm}
      \begin{enumerate}[1.]
        \item Removal of atypical (non-standard amino acids, more than one cleavage site) or poorly annotated records from data set of proteins with signal peptide from UniProt database,
...
%new column ------------------------------------------------------    
    ...
    3816 eukaryotic proteins with experimentally confirmed signal peptides and 9795 eukaryotic proteins without signal peptides downloaded from UniProt database release 2014\_07.
    ...
    \begin{enumerate}[1.]
      \item Chose randomly (without replacement) 3816 proteins without signal peptides and train the algorithm (called signal.hsmm) and reshuffle 3816 proteins with signal peptides.
...
metrics <- t(poster_data[["metrics"]][c("AUC", "H", "Gini", "Sens", "Spec", "Precision"), ])
...
levels(melted_metrics[["metric"]]) <- c("AUC", "H-measure", "Gini", "Sensitivity", "Specificity", "Precision")
...
@
    ...
    \begin{block}{Cleavage site prediction}
    \begin{figure}
    \scalebox{0.88}{    
<<echo = FALSE,message=FALSE,fig.width=14,fig.align='center'>>=
